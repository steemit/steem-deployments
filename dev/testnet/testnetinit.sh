...
# get latest snapshot from s3, used by tinman txgen - use only if .actions file has not already been created
#aws s3 cp s3://$S3_BUCKET/snapshot.json $HOME/snapshot.json

# the following is expected to change when actions file is autogenerated by gigantor
...
# NEW use when chainid is derived from git commit# tinman submit --realtime -t http://127.0.0.1:9990 --signer $UTILS/sign_transaction -c $CHAIN_ID --timeout 600
...
# NEW wait for seed to be synced before proceeding
